...
Partial Class TaxayerIndivitualReort
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        If DropDownList1.SelectedIndex = 1 Then
            ...
            sql = "select * from TaxReg where ID='" & TextBox1.Text & "'"
            da = New SqlDataAdapter(sql, cn)
            ds.Clear()
            da.Fill(ds, "ali")
            rpt.Load(Server.MapPath("TaxpayerCrystalReport.rpt"))
            rpt.SetDataSource(ds.Tables("ali"))
            CrystalReportViewer1.ReportSource = rpt
        ElseIf DropDownList1.SelectedIndex = 2 Then

            sql = "select * from TaxReg where departmentName='" & TextBox1.Text & "'"
            da = New SqlDataAdapter(sql, cn)
            ds.Clear()
            da.Fill(ds, "ali")
            rpt.Load(Server.MapPath("TaxpayerCrystalReport.rpt"))
